...
每天一个方块，**当天 token 越多，方块越亮（绿色梯度）、也越实心（`· ░ ▒ ▓ █`）**，
5 档梯度，附「少→多」图例，和 GitHub 首页的贡献墙一个意思。空档表示那天没有记录。

> 用**颜色 + 字符密度双通道**表示深浅：即便终端不支持 256 色、或绿色档位相近看不清，
> 也能靠方块的填充密度（点→半格→实心块）一眼分辨多少。
