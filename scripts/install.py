...
做了什么：
  1. 复制 log_token_usage.py / token_stats.py / token_stats_by_period.py 到 ~/.claude/
  2. 在 ~/.claude/settings.json 的 hooks.Stop 里注册 log_token_usage.py（去重，不覆盖已有其它 hook）
  3. 备份改动前的 settings.json 到 settings.json.bak
...
SCRIPTS = ["log_token_usage.py", "token_stats.py", "token_stats_by_period.py"]
